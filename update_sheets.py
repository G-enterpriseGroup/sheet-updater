...
for ticker in tickers:
    ...
    try: spreadsheet.del_worksheet(spreadsheet.worksheet(ticker))
    except: pass
    ...
    rows_by_exp = {exp: int(sub["Max Loss (Last)"].idxmax())
...
if summary_rows:
    ...
    try: spreadsheet.del_worksheet(spreadsheet.worksheet("Summary"))
    except: pass
    ...
    # Batch-format Summary with dynamic ticker colors
    ...
    # Build conditional requests for Summary
    req2 = []
    # Header styling
    req2.append({
        "repeatCell": {
    ...
    # Dynamic row coloring by ticker
        ...
        for ridx in indices:
            req2.append({
                "repeatCell": {
                    "range": {
                        "sheetId": sid2,
                        "startRowIndex": int(ridx)+1,
                        "endRowIndex": int(ridx)+2,
                        "startColumnIndex": 0,
